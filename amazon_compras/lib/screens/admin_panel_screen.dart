...
// Obtener categorías y subcategorías desde Firestore
// Reemplaza la función _getCategoriesWithSubcategories por esta:
...
Future<Map<String, List<Map<String, dynamic>>>> _getCategoriesWithSubcategories() async {
  ...
  // Primero, obtener categorías padre
  ...
  for (var padre in categoriasPadre) {
    ...
    // Buscar subcategorías de esta categoría padre
    ...
    result[padreNombre] = subcategorias; // ✅ Ahora es del mismo tipo
  }
  
  return result;
}

  // ==================== TAB 2: CATEGORÍAS CON IMÁGENES ====================
  ...
  // Método rápido para agregar subcategoría a una categoría existente
void _showAddSubcategoryDialog(String parentCategoryId, String parentCategoryName) {
  ...
  showDialog(
    ...
    builder: (ctx) => AlertDialog(
      title: Row(
        children: [
          const Icon(Icons.subdirectory_arrow_right, color: Color(0xFFFF9900)),
          const SizedBox(width: 8),
          Text(
            "Agregar Subcategoría a $parentCategoryName",
        ...
        ElevatedButton(
          onPressed: () async {
            ...
            if (context.mounted) {
              Navigator.pop(ctx);
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text("✅ Subcategoría agregada"), backgroundColor: Colors.green),
              );
              setState(() {});
            }
          },
        ...
        builder: (context, setDialogState) {
          return FutureBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              // Validar que selectedPadreId exista en la lista de categoriasPadre
              ...
              return AlertDialog(
                ...
                // ✅ CONTENT CORREGIDO - Ancho fijo en lugar de porcentaje
                content: Container(
                  width: 450, // Ancho fijo en lugar de porcentaje
        ...
        builder: (context, setDialogState) {
          return FutureBuilder<QuerySnapshot>(
            ...
            builder: (context, snapshot) {
              ...
              // Validar que selectedUserId exista en la lista
  ...
  if (confirm == true) {
    try {
      // ✅ Primero obtener datos, luego eliminar
 ...
 Widget _buildStatusDropdown(String orderId, String currentStatus) {
  ...
  // Asegurar que currentStatus sea válido
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              // Send to all users by using userId='all'
              await NotificationService().createNotification(
                userId: 'all',
                title: title,
                body: body,
